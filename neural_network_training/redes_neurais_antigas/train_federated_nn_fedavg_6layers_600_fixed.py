...
Example:
    ...
    python train_federated_nn_fedavg_6layers_600.py   --partition ML-EdgeIIoT-FEDT/dominant_client   --label-col Attack_type_6   --num-rounds 40   --num-clients 10   --epochs 5   --batch-size 64   --learning-rate 0.001   --scaler global   --optimizer sgd   --momentum 0.9

python train_federated_nn_fedavg_6layers_600_fixed.py \
...
FEDAVG_MAX_EPOCHS = 200

# 6-layer MLP (FedAvg baseline): 5 hidden layers (sum=600 neurons) + output layer
FEDAVG_HIDDEN_SIZES = (256, 128, 128, 64, 24)  # sum=600


class FedAvgMLP600(nn.Module):
    """Fully-connected network used as a FedAvg baseline.

    - Six linear layers in total (5 hidden + 1 output).
    - Total hidden neurons: 600 (256+128+128+64+24).
...
def _set_model_params(model: nn.Module, params: List[np.ndarray]) -> None:
    ...
    for (key, ref_tensor), arr in zip(state_dict.items(), params, strict=True):
        ...
        # Match device (important when model is on CUDA)
        if t.device != ref_tensor.device:
            t = t.to(device=ref_tensor.device)
...
class FlowerClient(fl.client.NumPyClient):
    def __init__(
        ...
        self.data = data
        self.model = FedAvgMLP600(input_dim=input_dim, num_classes=num_classes)

        # "Estilo Flower docs": move modelo para GPU se existir
                # Use CUDA only if it's available AND visible to this process (e.g., Ray actor assigned a GPU)
        cuda_visible = os.environ.get("CUDA_VISIBLE_DEVICES", "").strip()
        use_cuda = torch.cuda.is_available() and cuda_visible not in ("", "-1")
...
def main() -> None:
    ...
    parser.add_argument("--epochs", type=int, default=200, help="Local epochs per round (capped at 200 to match FedAvg baseline).")
    ...
    choices=["adam", "sgd"],
    default="sgd",
    help="Otimizador local (baseline FedAvg: SGD).",
)
    parser.add_argument(
    "--momentum",
    type=float,
    ...
    # Enforce the "max 200 epochs per round" FedAvg baseline constraint
    if args.epochs > FEDAVG_MAX_EPOCHS:
        print(f"[WARN] --epochs={args.epochs} > {FEDAVG_MAX_EPOCHS}; capping to {FEDAVG_MAX_EPOCHS} (FedAvg baseline).")
        args.epochs = FEDAVG_MAX_EPOCHS
